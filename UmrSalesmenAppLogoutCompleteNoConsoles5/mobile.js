...
api.post("/salesmanSignup", function (req, res) {
    ...
    ref.createUser({
        ...
        if (error) {
            ...
            var salemanSignUpSchema = new schemas_2.salesman({
            ...
            salemanSignUpSchema.save(function (err, data) {
                if (err) {
                    ...
                    schemas_1.company.findOne({ salemanMongoId: 'notAvailableDueToSlowInternet' }, function (err, derr) {
...
api.post("/signIn", function (req, res) {
    ...
    ref.authWithPassword({
        ...
        if (error) {
            ...
            schemas_2.salesman.findOne({ uid: authData.uid }, function (err, data) {
                if (err) {
                    console.log("Error in finding User", err);
                }
                else {
...
api.post('/dashboard', function (req, res) {
    console.log(req.body);
    /*include data in schema  */
    var salemanOrderMongo = new schemas_3.salemanOrder({
    ...
    salemanOrderMongo.save(function (err, data) {
        if (err) {
            ...
            schemas_2.salesman.findOne({ _id: req.body.salemanMongoId }, function (err, doc) {
